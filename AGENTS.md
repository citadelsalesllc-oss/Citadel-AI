...
1. **`Orchestrator.handle(request)`** — free-text entry point:
   ...
   2. Classifies the instruction (`classifyRequest`, `agents/src/orchestrator/router.ts`) — deterministic keyword routing to either the content-generation skill or one of the six specialist agents below (all still stubs from this entry point — see "SEO Agent," "Review Agents," and "Website Agent" below for why even the implemented ones are reached only through their structured entry points here).
...
5. **`Orchestrator.runWebsiteAudit(request)`** (Phase 7) — structured entry point for `POST /clients/:clientId/ai/website-audit`: takes an explicit `{ task, url, targetService?, targetLocation?, userInstructions? }` (same shape as `runSeoAudit`) and delegates the same way.

All structured entry points resolve `task` through the same private `resolveSkillForTask()` lookup (`TASK_SKILL_MAP` in `prompts/orchestrator/v1.ts` — `create_social_post -> create-social-post`, `seo_audit -> seo-audit`, `website_audit -> website-audit`, `review_analyze -> review-analyze`, `review_response -> review-respond`); anything not in that map throws `NotImplementedError` rather than guessing at a mapping. See ARCHITECTURE.md "Structured AI generation pipeline," "SEO analysis pipeline," "Website Intelligence pipeline," and "Review Intelligence pipeline."

Either way, the Orchestrator does not generate content, run QA, fetch a website, look up a review, or touch the database itself — see `create-social-post`/`seo-audit`/`website-audit`/`review-analyze`/`review-respond` in `skills/` for where that actually happens.
...
`website_fetch` (`tools/src/website-tools.ts`) predates this implementation and is the same tool the Website Agent below reuses to retrieve a page; `website_analyze` remains registered but unused by either real agent. The SEO Agent's own checks (`agents/src/seo/checks.ts`) are a separate, much deeper deterministic engine built directly for it, not layered on `website_analyze` — and the Website Agent's checks (`agents/src/website/checks.ts`) directly call into `seo/checks.ts`'s conversion checks rather than re-detecting the same CTA/phone/trust-signal evidence a second time; see "Website Agent" below and ARCHITECTURE.md "Website Intelligence pipeline" for how the two divide responsibility (search visibility vs. marketing/conversion effectiveness).
...
## Website Agent (`agents/src/website/`) — **implemented** (structured entry point) / **stub** (free-text entry point)

`WebsiteAgent.run()` audits one already-fetched webpage (`{ url, page, targetService?, targetLocation?, userInstructions? }`) and returns the full structured `WebsiteAuditResult` — first impression, conversion, customer journey, content, brand, an honest mobile-not-tested disclosure, and prioritized recommendations, each required to cite at least one evidence id. It is deliberately not another SEO checker: where the SEO Agent asks "can this page be found," the Website Agent asks "does this page turn a visitor into a customer" — see ARCHITECTURE.md "Website Intelligence pipeline" for the deterministic checks (`agents/src/website/checks.ts`, which calls `agents/src/seo/checks.ts`'s conversion checks directly rather than re-detecting the same CTA/phone/trust-signal evidence) plus the LLM-prioritized recommendations layered on top. Reached exclusively through the `website-audit` skill (`POST /clients/:clientId/ai/website-audit`) — the same relationship `SeoAgent` has to `seo-audit`.

The free-text `Orchestrator.handle()` router still points website-flavored instructions ("Run a website audit...", "How's our conversion...") at a **stub** (`agents/src/website/index.ts`'s `websiteAgent`, registered in `agent-registry.ts`), because that entry point has no way to extract a URL out of free text and `website_audit` requires one — the same reasoning as the SEO Agent's free-text stub above. Both share the name `'website-agent'` deliberately: they're the same capability, reached through two different, equally real entry points.
