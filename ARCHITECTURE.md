...
## Citadel Command Center dashboard (Phase 6)

The Command Center is an **internal staff tool** — not a client portal, not billing — for reviewing and approving what every specialist agent has produced. It is deliberately NOT a new specialist agent or a new capability: every fact and action it exposes already existed (client knowledge, content, SEO audits, reviews, the approval-lifecycle tools); Phase 6 only adds a way for a human to see and act on it across every client at once.

**Two packages, one business-logic layer.** `apps/api/src/routes/dashboard.ts` is a new router mounted at `/dashboard` in the SAME Express app as every other route (`app.ts`) — there is no separate dashboard backend service. It reuses the existing repositories directly for reads and the existing tenant-scoped tools (`approval_request`, `content_approve`, `content_reject`, `content_request_revision`) for content actions, so the platform still has exactly one place that knows how to validate an approval transition and record an audit entry. `apps/dashboard` (`@citadel/dashboard`) is a second, genuinely separate workspace package: a minimal static-file Express server (`src/server.ts`, its own `DASHBOARD_PORT`) with no build tooling or frontend framework, serving a vanilla HTML/CSS/JS single-page app (`public/`) that calls `apps/api`'s `/dashboard/*` JSON endpoints via `fetch()`, cross-origin (`apps/api` already runs `cors()` globally). This split was chosen over building the dashboard's data layer as its own service to avoid a second, competing source of truth, and over embedding the frontend inside `apps/api` to keep the already-reserved `apps/dashboard` slot doing exactly what its name says.

**Review-response approval didn't exist before Phase 6 — content approval did.** `tools/src/approval-tools.ts`'s content tools (`approval_request`/`content_approve`/`content_reject`/`content_request_revision`) already existed from Phase 1; Phase 6 adds their direct equivalents for review responses (`review_approve`/`review_reject`/`review_request_revision`, `tools/src/review-tools.ts`), following the identical shape — resolve the tenant, run the transition through `reviewRepository.transitionStatus()` (a status-only sibling of `saveResponse()` that doesn't touch response text or append a version, since approving a response doesn't change what it says), and record an `AuditLog` entry. This is why the master spec's "equivalent review-response approval endpoints should be provided" was satisfiable without inventing a new pattern — it's the same one, applied to the one lifecycle that hadn't needed it yet.

**One click, two real transitions.** `ContentStatus`'s state machine (`database/src/repositories/content-repository.ts`) only reaches `APPROVED`/`REJECTED`/`REVISION_REQUIRED` from `REVIEW`, not directly from `DRAFT`. Rather than adding a shortcut transition (which would mean the dashboard and the rest of the platform disagree about what states are reachable), the dashboard's approve/reject/revision routes call `approval_request` first when the item isn't already in `REVIEW`, then the target transition — two real, audit-logged transitions for what a staff member experiences as one button. Review-response approval needed no such chaining: its existing transition table already allows `DRAFT -> APPROVED/REJECTED/REVISION_REQUIRED` directly.

**Version history is append-only, mirroring `ReviewResponseVersion` from Phase 5.** `ContentVersion` (new in Phase 6) stores one row per version of a content item's body; the first row (`source: AI_GENERATED`) is written atomically with the `ContentItem` itself, and `contentRepository.editContent()` — called only from the dashboard, never by an agent or tool — appends another (`source: HUMAN_EDIT`) rather than overwriting `body` in place. `ReviewResponseVersion` gained the same `source` field retroactively (existing/historical rows default to `AI_GENERATED`) so both version-history systems are symmetric. Editing never changes lifecycle status for content (`editContent` leaves `status` untouched — an edit is "the draft reads differently now," not an approval decision); editing a review response always lands it at `DRAFT`, since a human just produced the text that's now ready for approval, and that transition is enforced through the review's own `assertTransition` guard (so editing an already-`APPROVED`/`REJECTED`/`PUBLISHED` response is rejected with the same structured `InvalidLifecycleTransitionError` as any other illegal transition, rather than a bespoke check).

**Cross-tenant reads are a deliberate, narrowly-scoped exception — writes are not.** Every other layer of this platform enforces "an id that's valid but belongs to another client is indistinguishable from an id that doesn't exist" (see "Tenant isolation" in SECURITY.md). An internal command center inherently breaks that assumption for READS: staff need to see every client's content, reviews, and audits in one queue. Each repository gained a `listAllForDashboard()`/`requireByIdGlobal()` pair, clearly marked `DASHBOARD-ONLY` in its doc comment, that is unscoped by `clientId` — and is never registered as an agent-callable `Tool`. Critically, every WRITE the dashboard performs still derives the authoritative `clientId` from the fetched record itself (`item.clientId`, `review.clientId`) before calling a tenant-scoped tool or repository method; the dashboard's HTTP routes never accept a caller-supplied `clientId` for a write. A content id or review id is the only thing a dashboard route trusts from the request — which client it belongs to is always looked up, never asserted by the caller.

**The AI Activity feed needed a real, queryable table — `console.log` alone wasn't enough.** `apps/api/src/logger.ts`'s `logGenerationEvent`/`logSeoAuditEvent`/`logReviewEvent` (Phases 3–5) only ever wrote structured JSON lines to stdout — nothing a dashboard could query. Phase 6 adds an `ActivityLog` Prisma model and `activityLogRepository`, and extends those same three logger functions to also persist a row (`clientId`, `requestId`, `agent`, `task`, `modelProvider`, `executionTimeMs`, `success`, `errorCode`, and a small `metadata` object) — deliberately the same no-secrets field set as the console line, never generated content, review text, or credentials. A generation/audit/review event logged against a client id or slug that doesn't resolve to a real client (e.g. a request for a nonexistent client) would otherwise fail the table's foreign key; `activityLogRepository.record()` retries once with `clientId: null` in that case rather than silently dropping the event, so a failed request's activity entry is never lost.

**SEO audits have no dashboard-mutable state.** Per the master spec ("do not allow dashboard edits to alter the original audit result"), `seoAuditRepository`'s dashboard additions are read-only (`listAllForDashboard`/`requireByIdGlobal`) — there is no `editAudit` and no approve/reject action for an audit, because an audit is a point-in-time finding, not a draft awaiting approval.

**Not built in Phase 6, per the master spec:** the Website Agent, Google Business/Facebook publishing integrations, autonomous publishing, Google OAuth, billing, a public client portal, and a vector database. The dashboard shows what already exists; it does not add new AI capabilities or new external integrations.

**OpenClaw is prepared for, not integrated.** The AI Activity feed's shape (`agent`, `task`, `clientId`, timing, success/failure) is generic enough that a future OpenClaw-originated action could write the same kind of `ActivityLog` row and appear in the same feed with no schema change — but nothing in Phase 6 calls OpenClaw or feeds its output anywhere. See OPENCLAW.md.

**Authentication reuses what already exists — see SECURITY.md "Command Center authentication boundary."** The dashboard introduces no new auth code: `/dashboard/*` sits behind the exact same `createAuthMiddleware`/`actorMiddleware` chain as every other route in `app.ts`, proven in `apps/api/src/__tests__/dashboard.test.ts` ("authentication boundary").

## Website Intelligence pipeline (Phase 7)

The fifth specialist agent, `WebsiteAgent` (`agents/src/website/website-agent.ts`), follows the same "deterministic engine does the measurable work, the model only prioritizes and explains" split as `SeoAgent` — but it is deliberately not a second SEO checker. Where `SeoAgent` answers "can this page be found," `WebsiteAgent` answers "does this page turn a visitor into a customer": first impression, conversion, customer journey, content quality, and brand consistency, evaluated from a marketing and UX perspective rather than a search-visibility one.

A fifth structured entry point, `Orchestrator.runWebsiteAudit(request)`, sits alongside the other four, exposed over HTTP as `POST /clients/:clientId/ai/website-audit`:

```
POST /clients/:clientId/ai/website-audit
  -> Orchestrator.runWebsiteAudit()
       -> identifyAndValidateClient()          (client_context tool; ClientNotFoundError / ClientNotActiveError)
       -> resolveSkillForTask('website_audit')  (TASK_SKILL_MAP lookup — shared with generateContent()/runSeoAudit(), see above)
       -> SkillRegistry 'website-audit'
            -> website_fetch tool                (the SAME tool seo-audit uses — one fetch adapter, two interpretations)
            -> WebsiteAgent.run()
                 -> 5 deterministic check categories (agents/src/website/checks.ts) -> first_impression/conversion/customer_journey/content/brand scores + an evidence catalog
                 -> ModelProvider.generate() with the evidence catalog as input, responseSchema = WebsiteInterpretationResultSchema
                 -> evidence-ref validation: any recommendation citing an id not in the real catalog is dropped
                 -> quickWins/highImpactChanges derived server-side by filtering the model's single recommendation list (never asked for three times)
            -> website_audit_save tool            (persists the full result; never gated on a pass/fail — every audit is worth keeping)
  -> structured response: { audit, evidence, recommendations, quick_wins, high_impact_changes, clientId, agentUsed, modelProvider, usage, executionTimeMs }
```

**Website vs. SEO: shared ingestion, divergent analysis.** Both agents fetch through the identical `website_fetch` tool/`WebsiteFetchAdapter` — there is no second, marketing-flavored fetch. `WebsiteFetchResult` (`shared/src/website-fetch.ts`) gained four fields in Phase 7 (`telLinks`, `mailtoLinks`, `formCount`, `phoneNumberMatches`) that Phase 4's SEO-era extraction deliberately didn't need — `extractLinks()` filtered `tel:`/`mailto:` hrefs out entirely, and no form-detection existed at all, since neither mattered for search visibility. Where the two agents' checks overlap (CTA presence, phone visibility, contact path, trust/review signals), `agents/src/website/checks.ts`'s `runConversionChecks()` calls `agents/src/seo/checks.ts`'s conversion checks directly and layers new, conversion-specific checks (form presence, click-to-call via `tel:` links, CTA repetition count, guarantee/certification/experience/financing signals, service-area-coverage fraction) on top — the concrete mechanism behind "do not duplicate the SEO Agent unnecessarily; where overlap exists, reference the existing SEO analysis instead of rebuilding it."

**The five deterministic check categories** (`agents/src/website/checks.ts`) — first impression (title/H1 presence, company/service clarity in the headline, target-audience and service-area obviousness), conversion (as above), customer journey (synthesizes friction points by reframing first-impression issues rather than re-detecting them, contact-path presence, "what happens next" language, FAQ presence), content (thin-content word count, service-mention coverage, benefit-vs-feature language, objection-handling and differentiation signals, duplicate-sentence detection, active-offers mentioned), and brand (forbidden-phrase presence, company-name and positioning consistency against the client's `brandProfile` — returns a neutral all-clear immediately if the client has no brand profile on file, since there's nothing to compare against) — each return `{ score, strengths, issues, evidence }` (customer journey uses `frictionPoints` in place of `issues`; brand omits `strengths` entirely, since its only job is flagging mismatches, never praising conformance — the output schema, `shared/src/types/website-audit.ts`, reflects this per-category shape variation deliberately rather than forcing one uniform shape). Guarantee/certification/experience/financing signals are only ever reported as strengths when actually found in the fetched content — their absence is never flagged as an issue, since a business legitimately may not offer financing at all, and "not present" is not "the site failed to communicate it."

**Mobile is disclosed honestly, never fabricated.** The fetch infrastructure retrieves HTML and text only — it never renders a page or takes a screenshot — so `WebsiteAuditResult.mobile` (`WebsiteMobileDisclosureSchema`) is structurally incapable of claiming mobile testing happened: `tested` is typed `z.literal(false)`, and `buildMobileDisclosure()` is a pure function that always returns the same honest note ("Mobile visual/responsive-layout testing was not performed..."). This is the concrete enforcement of "never pretend to have visually inspected a mobile layout without evidence" — a type-level guarantee, not a prompt instruction the model could ignore.

**The model never re-derives measurable facts — it only prioritizes and explains, and never invents numbers.** `WebsiteAgent.run()` hands the model the combined evidence catalog (never the raw page) via `prompts/website/v1.ts`'s `buildWebsiteUserPrompt()`, and asks for `{ recommendations, summary }` — each recommendation must cite `evidence_refs` from the catalog and carry a category (`CONVERSION`/`CONTENT`/`UX`/`BRAND`/`LOCAL`/`SEO`/`TRUST`/`CUSTOMER_JOURNEY`), a priority, a qualitative `impact` (`'HIGH IMPACT'`/`'MEDIUM IMPACT'`/`'LOW IMPACT'` — never a fabricated percentage; `SAFETY_REQUIREMENTS` explicitly forbids "this will increase conversions by 30%"-style claims unless real measured data exists, which it never does here), and an effort estimate. `WebsiteAgent` then filters exactly as `SeoAgent` does: any `evidence_refs` entry that isn't a real catalog id is dropped, and a recommendation left with zero valid refs is dropped entirely.

**Persistence mirrors `SeoAudit` exactly, on purpose.** `WebsiteAudit` (Prisma) and `websiteAuditRepository` follow the identical append-only shape and rationale as `SeoAudit`/`seoAuditRepository` — every completed audit is saved regardless of score, never overwritten, so `website_audit_history` (and the dashboard) can support before/after comparison as a client's conversion effectiveness improves over time.

**The Command Center dashboard was extended, not duplicated, for Phase 7.** A new `Website` tab (`apps/dashboard/public/`) lists audits (client/URL/score/date/agent-version) and a detail view renders all five category panels, the mobile disclosure, quick wins, high-impact changes, and evidence — read-only, same as SEO audits ("do not allow editing of the original audit result"). `GET /dashboard/website-audits` and `GET /dashboard/website-audits/:auditId` mirror the SEO dashboard routes exactly, and the overview endpoint's `recentWebsiteAudits` sits alongside `recentSeoAudits` without altering it.

**Test fixtures are fictional and clearly marked.** `integrations/src/websites/fixtures.ts` holds six HTML fixtures for fictional local-service businesses (strong conversion, weak conversion, missing CTA, weak service messaging, strong-SEO-weak-conversion, strong-conversion-weak-SEO), each documented in the file's own doc comment as TEST/DEVELOPMENT data — the same "never invent real businesses or customer information" rule already applied to seed and review fixture data.
