import type { ModelProvider } from '@citadel/shared';
...
export function createModelProvider(config: ModelProviderConfig): ModelProvider {
  ...
  if (requested === 'anthropic') {
    if (!config.anthropicApiKey) {
      throw new Error(
        'MODEL_PROVIDER=anthropic requires ANTHROPIC_API_KEY to be set. Use MODEL_PROVIDER=mock for local development without a key.',
      );
    }
